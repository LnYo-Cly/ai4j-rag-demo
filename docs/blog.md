...
**demo 没用 SDK 自带的 `ChatRagJudge`，而是自己实现了 `AnthropicRagJudge`——这本身就是 `RagJudge` SPI 的最佳例证。** 原因是一个真实的坑：`ChatRagJudge` 走 **OpenAI 协议**（`IChatService`）→ 必须打 GLM 的 `paas/v4` 入口；而 demo 主答走 **Anthropic 协议**，用 `api/anthropic` 入口。GLM 的 coding-plan key **只对 anthropic 入口有效**，拿它打 paas/v4 会报余额不足（实测 `HTTP 429 code 1113 余额不足`）。所以 demo 实现 `RagJudge` 接口、复用主答的 anthropic 通道——换通道不换算法，judge 照样跑：

```java
// RagJudge 是 SPI 接口——demo 自实现 AnthropicRagJudge 复用 anthropic 通道（coding-plan key 有效），
// 绕开 ChatRagJudge 要求的 paas/v4（OpenAI 协议，coding-plan key 打不通）。
RagOnlineEvaluator evaluator = new RagOnlineEvaluator(
        new AnthropicRagJudge(messagesService, "glm-4.6"));
...
多轮场景（11.3 的「它的时效是多久」，query 含模糊指代）judge 正确降分到 `contextRelevanceScore: 0.8`，reason 标注「vague query」—— **分数有区分度，不是无脑全 1.0**。
...
`RagJudge` 是接口（对称 `Reranker` 可换 jina/llm）—— 不锁死算法/通道，想接 Ragas、换更强 judge 模型、或像 demo 这样换协议通道，实现接口即可。
...
开 `rag.online-eval-enabled=true` 后，每答自动多一次 GLM-as-judge 打分（走 demo 自实现的 `AnthropicRagJudge`，复用主答 anthropic 通道）。退款单轮问答实测响应：
```json
{
