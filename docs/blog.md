...
> **怎么读**（22 章，按需跳读，不必顺序）：
> - **只看效果** → 十八（真实运行结果：多租户隔离 / 检索评估 / agent trace）
> - **要上生产** → 十四（高并发）+ 十五（多租户安全）+ 十七（部署）+ 二十一（上线检查清单）
> - **动手搭一套** → 九～十三（项目结构 → 摄入 → 问答 → 索引 → 生产代码）
> - **懂原理 + 设计** → 四（embedding/RAG 本质）+ 七（12 条设计原则）
> - **踩坑/选型参考** → 十九（坑位）+ 二十二（ai4j 真实不足）
>
> 主线：**离线摄入 → 在线检索/重排/生成 → 多租户/可观测/部署 → 真实案例 → 坑位与总结**。
...
### 4.1 embedding
把文本映射到高维稠密向量空间，让语义相近的文本距离更近。**这个"语义距离"不是手工设计的**——embedding 模型在海量语料上训练，让经常共现 / 上下文相似的文本落到相近的位置，语义相似性是从训练里涌现的。查询时把问题也转向量，用相似度（Cosine / IP / L2）找最近邻 chunk。本文用 Qwen3-Embedding-0.6B（1024 维，中文好）。
...
│ +ChatReranker(GLM打分)│    │  生成 + ChatReranker 复用   │
│ +ContextAssembler     │    └───────────────────────────┘
└──────────────────────┘
        │
        ▼
...
| AI SDK | ai4j 2.4.2（starter，Central 已发布，clone-and-run 无需构建 SDK） |
| Embedding | Ollama + Qwen3-Embedding-0.6B（1024 维） |
| Chat | GLM（Anthropic Messages，`IMessagesService`） |
| 向量库 | PostgreSQL + pgvector |
| 重排 | ChatReranker（SDK 2.4.2，GLM 打分；专用 rerank 用 ai4j `ModelReranker` 接 Jina/Doubao） |
...
└── src/main/
    ...
    │   ├── config/   (RagProperties, PgVectorSchemaInitializer, RagMetrics)
    │   ├── domain/   (ChatRequest, RagAnswer, ReferenceItem)
    │   ├── service/  (KnowledgeIngestionService, RagQueryService, QueryRewriteService,
    │   │              InMemoryCorpus, MockOrderService, MockTicketService)
    │   └── controller/ (ChatController, EvaluationController, AgentController,
...
### 10.5 增量 ingest：content-hash 跳过未变更 chunk（省 embed）

10.3 的「重启幂等」靠 upsert（同 chunkId 覆盖）—— 但**每次重启都重新 embed 全部 chunk**（embed 是成本大头），浪费。生产知识库大时（几万 chunk），全量 re-embed 慢且贵。

ai4j 的 `IngestionPipeline` 支持 **content-hash 增量**：自动给每个 chunk 的 metadata 写 `contentHash = SHA-256(content)`；`IngestionRequest.skipExistingContentHash=true` 时，upsert 前先 `vectorStore.exists(filter={contentHash})` 查重 —— 已存在就 skip（**embed + upsert 两步都省**），`IngestionResult.skippedCount` 统计跳过数：
...
- `llm`（默认）：SDK 2.4.2 `ChatReranker`（`getChatReranker(ANTHROPIC, model)`），用 GLM 对每个 hit 打分重排。**没有专用 rerank 模型时的兜底**——本地 Ollama 原生没有 `/api/rerank` 端点（实测 404），所以用 chat 模型打分是通用替代。
...
private Reranker resolveReranker(AiService aiService) {
    ...
    return aiService.getChatReranker(PlatformType.ANTHROPIC, ragProperties.getGlmModel(),
            ragProperties.getTopK(), null, true, 20);   // SDK 2.4.2 ChatReranker
}
```
...
ai4j 2.4.1 起把 history 接进 **retrieve 层**：`RagQuery` 加 `history: List<ChatMemoryItem>`，`ModelRagQueryPlanner.plan` 把 history 拼进 LLM prompt，生成 variant 时消解 follow-up 指代：
...
ai4j 2.4.1 的 `TokenAwareRagContextAssembler`：**token 级预算**（jtokkit 精确 BPE 计数），超 budget 停追加（reranked 高相关在前，保留前 N），第一个 hit 自己超长才截断（至少答一个，不全空），citations 只保留实际进 context 的来源：
...
encoding fallback 还做成三层（普通传模型名 / 高级 `withEncoding` / 未知模型降级不失败）—— 国产模型 jtokkit 不认时降级 `CL100K_BASE` BPE 近似，不抛异常。token 级预算比字符级精确，context 既不超 `max_tokens`，也不留余量浪费。
...
public RagQueryService(AiService aiService, PgVectorStore vectorStore, RagProperties props) {
    ...
    this.ragService = new DefaultRagService(
            ...
            aiService.getChatReranker(PlatformType.ANTHROPIC, props.getGlmModel(),
                    props.getTopK(), null, true, 20),   // SDK 2.4.2 ChatReranker
...
### 13.5 ChatReranker（SDK 2.4.2，无专用 rerank 时的兜底）
没有专用 rerank 模型时，用 chat 模型对召回结果打分重排——SDK 2.4.2 的 `ChatReranker` 内置了这个能力（LLM-as-reranker via `IChatService`），一行工厂调用，不用自己实现 `Reranker`：
```java
// 走 PlatformType.ANTHROPIC → AnthropicChatService → coding-plan key 可用
Reranker reranker = aiService.getChatReranker(PlatformType.ANTHROPIC, "glm-4.6",
        topK, null, true, 20);   // model, topN, instruction, appendRemainingHits, maxCandidates
// 内部：构造 query+每个 hit 的 prompt → GLM 返回 [{index,score}] JSON → 按分排序写回 rerankScore
//      失败软降级（chat 抛错/坏 JSON/空 → 原 hits 顺序）
```
生产有专用 rerank（Jina/Doubao）时换 `aiService.getModelReranker(platform, model)`，主链不变（都是 `Reranker` 接口）。
...
这就是 `RagTool` 的价值——一行 `.capture(sink)` 把整条 agent 链路（思考→调检索 tool→生成）全捕获，**RAG 不再是 agent 可观测之外的黑盒**：
...
| 路径 | token 来源 | cost |
|---|---|---|
| agent | IoCapture `NodeIoRecord.inputTokens/outputTokens`（per MODEL 节点） | `TraceMetrics`（token × pricing） |
| 独立 RAG | generate 响应 `usage.input_tokens/output_tokens`（GLM anthropic 实测回填） | 应用层算（token × 单价） |

demo 在 2.4.2 上跑，token 直接挂 `RagAnswer.inputTokens/outputTokens`（实测退款单轮 `inputTokens:50 outputTokens:251`，多轮带 history `inputTokens:499`）。SDK 2.4.2 另有 `RagGenerationUsage`（trace 级统一通道，含 inputCost/totalCost 字段）可挂到 `RagTrace.generationUsage`——demo 选 RagAnswer 字段是因为它直接出现在 HTTP 响应里（用户/前端可见），trace 级的留给需要重放/审计的场景。
...
KnowledgeIngestionService : Ingested [public] after-sales.md: upserted=1 skipped=0 | load=1ms chunk=0ms embed=39268ms upsert=343ms total=39613ms
KnowledgeIngestionService : Ingested [public] logistics.md: upserted=1 skipped=0 | load=0ms chunk=0ms embed=589ms upsert=105ms total=695ms
KnowledgeIngestionService : Ingested [public] refund-rules.md: upserted=1 skipped=0 | load=0ms chunk=0ms embed=414ms upsert=134ms total=548ms
KnowledgeIngestionService : Ingested [premium] marketing.md: upserted=1 skipped=0 | load=0ms chunk=0ms embed=423ms upsert=128ms total=552ms
KnowledgeIngestionService : Ingested [premium] merchant.md: upserted=1 skipped=0 | load=1ms chunk=0ms embed=401ms upsert=119ms total=521ms
KnowledgeIngestionService : Knowledge ingestion done: 5 chunks total
```
首 chunk embed 39 秒是 Ollama 把 embedding 模型加载进 VRAM 的冷启动开销（后续降到几百毫秒，见 19 坑位 14）。暖库重启时同样的日志会是 `upserted=0 skipped=1`——content-hash 增量跳过未变更 chunk（10.5），embed/upsert 两步都省。
...
### 18.5 重排（ChatReranker）
每个在线问答都过 `ChatReranker`（SDK 2.4.2，GLM 对召回的 hit 批量打分重排）——日志能看到 rerank 前后顺序变化，hit 带 `rerankScore`。Ollama 无 rerank 端点时这是通用兜底。
...
STEP 3  retrievedHits（retrieve 后、rerank 前）:
          ...
          - after-sales.md    score=0.353
          - logistics.md      score=0.314
STEP 4  rerankedHits （ChatReranker 后）:
          - refund-rules.md   rerankScore=0.491
          - after-sales.md    rerankScore=0.353
          - logistics.md      rerankScore=0.314
STEP 5  context      : [S1] refund-rules.md\n# 退款规则\n## 退款申请时效\n用户在订单签收后 7 天内…
...
> 注：上面 STEP 2 是 `planner-enabled=false` 时的样子（QueryRewriteService 把口语改写成正式术语）。demo 默认 `planner-enabled=true`——此时 `rewrittenQuery` 字段是原 query，改写体现在 `queryPlan.variants`（planner 生成多路 query 变体，见 11.1/11.3）；两种模式下后面 STEP 3-6 的结构一致。
...
### 18.8 在线评估 + token 用量（SDK 数据通道，实测）
...
8. **Ollama 无 `/api/rerank`** → 用 LLM 兜底重排（SDK 2.4.2 `ChatReranker`，`getChatReranker(ANTHROPIC, model)`），或接 cloud rerank（Jina/Doubao）
...
13. **依赖锁 SNAPSHOT 破坏 clone-and-run**：pom 用 `2.4.3-SNAPSHOT` 时，clone 下来的人拉不到（Central 没有）→ 必须先构建 SDK。demo 要锁 Central 已发布版本（2.4.2），保住开箱即用。
...
18. **agent 路径的 knowledge_search 跨租户泄漏**（demo 实测发现 + SDK 2.4.2 已补，**安全问题**）：主问答 `/api/rag/ask` 显式给 `RagQuery` 带 `permissionTag` filter（15 章），但 agent 路径用 `RagTool` 时——早期 `RagTool` 内部构造 `RagQuery` **不带 filter**，结果 default 租户经 agent 的 `knowledge_search` 能召回到 premium 专属文档（实测拿到了 `marketing.md`）。15 章说"RAG 最危险的是串库"，agent 路径恰恰漏了这层。**SDK 2.4.2 已闭合**：`RagTool.builder(ragService).filter(tenantFilter)` 服务端固定注入过滤（不进 tool schema、LLM 改不了）；demo 现在直接用它，`RagService`/retriever 变租户无关共享、只 `RagTool` 每请求带 filter。教训：多租户场景下，主链和 agent 链路的权限隔离必须**两条都查**——别假设 agent tool 会自动继承主链的 filter。
...
### 工程与发布（17 章、19 坑位）
- ☑ **依赖锁已发布版本**：pom 用 Central 上的 release（如 2.4.2），别用 SNAPSHOT——否则 clone-and-run 断（19 坑位）
...
3. ~~RAG-as-Agent-Tool 无现成封装~~ **已解决**：`RagTool` + `AgentBuilder.capture()` 让 RAG 作为 agent tool 接入统一可观测链路
4. **LLM 重排 / Ollama rerank**——Ollama 无 `/api/rerank` 端点（`OllamaRerankService` 在纯 Ollama 上 404）。**"只有 chat 模型也要能重排"这个缺口已在 SDK 2.4.2 由 `ChatReranker` 闭合**（LLM-as-reranker via `IChatService`）：任何 chat 平台（含 ANTHROPIC/coding-plan key）都能 `getChatReranker(platform, model)` 重排。demo 现在直接用 SDK 的 `ChatReranker`（走 `PlatformType.ANTHROPIC`），不再需要本地的 LLM-rerank 兜底类。
...
在线问答 POST /api/rag/ask
  ...
  → 否则：多租户 filter（permissionTag 前置）
       ...
       → ChatReranker（GLM 打分重排）
       → TokenAwareRagContextAssembler（token 预算，[S1][S2] 引用）
       → GLM generate（IMessagesService，拒答约束）
       → online eval 打分 + token 用量捕获
       → 答案 + references + scores + 写缓存
