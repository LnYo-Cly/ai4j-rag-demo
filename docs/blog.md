...
| 层 | 能力 | ai4j 提供 | demo 用 |
|---|---|---|---|
| **agent 级** | 运行时事件流：节点 I/O 捕获、重放、失败恢复、断点续跑、tamper-evident 审计 | ✅ `IoCaptureSink`（InMemory/Jsonl）+ `IoCaptureAgentListener`（PR #146-149 全 4 阶段） | demo 不走 agent runtime → 用不上（适用于 agent 场景） |
| **RAG 级** | 检索过程可解释 | ✅ `RagTrace`（retrievedHits/rerankedHits）+ `RagScoreDetail`（每 hit 融合分来源）+ `RagCitation` | ✅ demo 用 |
| **应用级** | 指标/缓存 | —（应用层） | ✅ `cacheStats()`（命中率/大小）+ Micrometer |

诚实：ai4j 的完整可观测系统（重放/恢复/审计）是 **agent runtime 级**，绑定 ReAct 循环；RAG 直调 `IMessagesService` 不走 agent，所以 RAG 场景的可观测落在 RagTrace + 应用层指标。如果 RAG 包成 agent tool 跑，就能用上 agent 级可观测。
...
- 单问答→智能体 → ai4j-agent（ReAct+工具+记忆+compaction）；⚠️ ai4j 当前无"RAG-as-Agent-Tool"现成封装，要自己把 `RagService` 包一层 tool
...
3. **RAG-as-Agent-Tool 无现成封装**——agent 没有现成 RAG tool，用户自己包一层
